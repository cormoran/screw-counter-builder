...
スライダーの脱落を防ぐため、クリックばねと反対側のベース側レールに上面開放の長い溝を設け、右端の1.1 mmをストッパーとして残す。スライダー側には10 mmのU字逃げ溝で横方向にたわむ舌部と凸を設ける。凸の挿入側は45度のカム、引き抜き側は垂直面とする。組立時は右側から押し込み、舌部をたわませてストッパーを越える。全列排出位置ではストッパーとの隙間を残し、さらに4分の1ピッチ引くと凸が当たることをCADで確認する。実物の押し込み力・脱着耐久性は未測定。
