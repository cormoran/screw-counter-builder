...
トレー取っ手の付け根は左右に幅3 mmの45度三角リブを追加する。各リブはトレー後壁に0.4 mm重ね、取っ手側へ2.8 mm伸ばす。フック穴は付け根から6 mm先に始まるためリブと重ならず、中央の開口も残る。印刷姿勢では斜面が上へ積み上がる形となる。CAD検証でリブ内の材料と斜面上の空間を確認する。吊り下げ時の曲げ強度は実機で確認する。
