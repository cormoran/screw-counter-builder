...
クリック感が弱いという実印刷の報告に対し、板ばねの有効長を既定15→13 mmに短縮する。既存の「ばね幅」に加え「ばね長さ」を設定できるようにし、12〜18 mmを許可する。長さはノーズ付近のX=12 mmから逃げ溝の終端までの距離で、短くすると逃げ溝が短くなり、ばねの剛性が上がる。操作力・疲労寿命は未測定のため、新しい既定値の実印刷確認が必要。
...
吊り下げ部の薄さを改善するため、トレー取っ手はデッキから上へ0.8 mm増厚して2.4 mmとし、両取っ手の穴を短辺方向8→6 mm、長辺方向で片側1 mmずつ縮めて穴の周囲を太くする。スライダー取っ手を下へ増厚すると印刷姿勢で本体が造形プレートから浮くため、厚さ2 mmと印刷面を維持する。両者の間の0.2 mmの摺動隙間は保つ。取っ手の材料と穴をCADのB-Rep交差体積で検証する。
...
排出口のトレー上端では、蓋が入る左右2つの角を約3 mmの45度斜面で広げる。切り欠きの下側とねじを案内するデッキは維持する。蓋のプラグ両側には斜面に沿う細いリブをつけ、斜面との間に約0.2 mmの組立余裕を設ける。蓋を閉じた状態の全パーツ干渉検証に加え、斜面上部でトレーが抜けて蓋リブが存在すること、さらに外側には隙間があること、斜面下部にはトレーの枠が残ることをB-Repで確認する。
