...
実印刷で逆向きのねじや同じ穴への重複が続いたため、Web版のトレー貫通穴の径余裕を `頭径+0.6` から `頭径+0.3 mm` に、上面入口の径余裕を `頭径+1.0` から `頭径+0.5 mm` に縮める。スライダーの取り出し窓は給送時の頭の通過を保つため従来の `頭径+1.0 mm` を維持する。穴を狭めても単純な円孔だけでは逆向きや重複を原理的に完全排除できないため、実測頭径を入力し、印刷と給送を試して確認する。

トレー貫通穴の径余裕は「クリアランス」内の `trayHoleClearance` で 0.1〜1.2 mm に調整できる。既定は0.3 mm。上面入口は貫通穴からさらに径0.2 mm広がる。頭径の実測値を維持したままプリンターの収縮や給送具合に合わせられる。
