...
排出口を塞ぐ蓋側の長い出っ張りは、蓋の前縁をX方向へ3.2 mm延ばし、そこから出っ張りの根元へ4.2 mmの45度三角リブをつなげる。リブは排出口幅内かつ主にトレー前面より外側に置き、閉じた状態のトレー・ベース・スライダーや磁石と干渉しない。蓋の既存の印刷向きでは斜面を上へ積むため、リブはサポートを要しない形状となる。CAD検証では斜面内に材料、斜面直下に空間があることを確認する。曲げ強度と印刷品質は実機で確認する。
