...
export function parseSettingsFile(contents: string): Settings {
  ...
  const raw: Record<string, unknown> = { lidStyle: DEFAULT_SETTINGS.lidStyle, funnelAlignment: DEFAULT_SETTINGS.funnelAlignment, funnelOutlet: DEFAULT_SETTINGS.funnelOutlet, ...document.settings as Record<string, unknown> }
  ...
  for (const key of ['detentSpringWidth', 'detentSpringLength', 'detentDiameter', 'magnetDiameter', 'magnetThickness', 'magnetDiameterClearance', 'magnetDepthClearance', 'slideClearance', 'trayHoleClearance', 'screwSpaceHeight'] as const) {
