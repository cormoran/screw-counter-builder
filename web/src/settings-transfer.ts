...
export function parseSettingsFile(contents: string): Settings {
  ...
  const raw: Record<string, unknown> = { screwLength: DEFAULT_SETTINGS.screwLength, trayStyle: DEFAULT_SETTINGS.trayStyle, lidStyle: DEFAULT_SETTINGS.lidStyle, funnelAlignment: DEFAULT_SETTINGS.funnelAlignment, funnelOutlet: DEFAULT_SETTINGS.funnelOutlet, funnelHeight: DEFAULT_SETTINGS.funnelHeight, ...document.settings as Record<string, unknown> }
  ...
  for (const key of ['headDiameter', 'shaftDiameter', 'slotWidth', 'pitch', 'funnelHeight'] as const) {
