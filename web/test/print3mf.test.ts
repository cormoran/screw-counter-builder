...
const meshes = Object.fromEntries(['base', 'tray', 'slider', 'lid', 'funnel'].map((part, index) => [part, triangle(index * 20)])) as Record<ModelPart, TriangleMesh>
const model = {
  partMeshes: meshes,
  files: Object.fromEntries(['base', 'tray', 'slider', 'lid', 'funnel'].map((part) => [`${part}.stl`, binaryStl(meshes[part as ModelPart])])),
} as GeneratedModel
...
describe('Bambu 3MF export', () => {
  ...
  it('creates a Bambu project with a centered Core 3MF build layout', async () => {
    ...
    expect(artifact.placements).toHaveLength(5)
    ...
    expect(xml.match(/<object /g)).toHaveLength(5)
  ...
  it('splits parts across Bambu plates and writes each plate mapping', async () => {
    const largeMeshes = Object.fromEntries(['base', 'tray', 'slider', 'lid', 'funnel'].map((part, index) => [part, triangle(index * 200)])) as Record<ModelPart, TriangleMesh>
    ...
    const largeModel = { partMeshes: largeMeshes, files: Object.fromEntries(['base', 'tray', 'slider', 'lid', 'funnel'].map((part) => [`${part}.stl`, binaryStl(largeMeshes[part as ModelPart])])) } as GeneratedModel
    const artifact = await createBambu3mf(largeModel, { width: 180, depth: 180 })
    expect(artifact.plates).toHaveLength(5)
    expect(artifact.plates.every((plate) => Object.keys(plate.previewMeshes).length === 1)).toBe(true)
    expect(artifact.placements.map((placement) => placement.plate)).toEqual([1, 2, 3, 4, 5])
    const archive = await JSZip.loadAsync(artifact.file)
    const settings = await archive.file('Metadata/model_settings.config')!.async('text')
    expect(settings.match(/<plate>/g)).toHaveLength(5)
