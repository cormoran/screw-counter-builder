...
describe("tray style CAD integration", () => {
  it("switches only the tray at 5 mm and exports both explicit styles", async () => {
    ...
    for (const part of ["base", "slider", "lid", "funnel"] as const) expect(holes.files[`${part}.stl`]).toBe(cutout.files[`${part}.stl`]);
